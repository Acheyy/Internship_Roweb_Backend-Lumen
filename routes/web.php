...
$router->group(['middleware' => ['cors', 'auth']], function () use ($router) {
    ...
    $router->get('/promotions', ['uses' => 'PromotionController@getAll']);
    $router->group(['prefix' => 'promotion'], function () use ($router) {
        $router->post('/', ['uses' => 'PromotionController@create']);
        $router->get('/{id}', ['uses' => 'PromotionController@get']);
        $router->post('/{id}', ['uses' => 'PromotionController@update']);
        $router->delete('/{id}', ['uses' => 'PromotionController@delete']);
    });

    $router->get('/sales', ['uses' => 'SalesController@getAll']);
});
